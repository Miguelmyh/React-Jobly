...
class JoblyApi {
  ...
  static async patchCompany(handle, data = {}) {
    //patch, post, delete require admin token
    //will need testAdmin token
  ...
  // users methods
  //get, post users require admin
  //every other route requires sameUserOrAdmin
  ...
  static async applyToJob(username, id, data = {}) {
    let res = await this.request(`${username}/jobs/${id}`, data, "post");
    return res;
  }
  ...
  //auth methods will be done when working on the login forms
}
